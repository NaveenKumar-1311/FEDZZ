...
def create_clients1(data, num_clients, initial, alpha):
    client_names = ['{}_{}'.format(initial, i+1) for i in range(num_clients)]
    training_data_size = len(data)
...
clients = create_clients1(train_sampler, num_clients=40,initial='clients', alpha=10)
...
GTAA=[]
cntr = 0
def execution(mutation, Zalloc,global_acc1, global_model, global_model1, global_model2, global_model3, global_model4, global_model5):
    global cntr
    global_epochs = 10
    local_epochs = 1
    oddclients = []
    ...
    for g_epoch in range(global_epochs):
        ...
        global_weights4 = global_model1.parameters()
        global_weights5 = global_model2.parameters()
            ...
            if int(client[8:]) in z1:
                ...
                for epoch in range(local_epochs):
                    ...
                    for batch_idx, (data, target) in enumerate(clients_batched[client]):
                        ...
                        if client == "clients_1" or client == "clients_3" or client == "clients_5" or client == "clients_7" or client == "clients_9" or client == "clients_11" or client == "clients_13" or client == "clients_15" or client == "clients_17" or client == "clients_19" or client == "clients_21" or client == "clients_23":
        ...
        # np.save('GTAA_40client_het_noattack_5z1AC510pzallrounds_18Jul2022.npy', GTAA)
        # np.save('./40c5z1A/selclientids_noattack_5z1A10pz_C5allrounds_772022_heter_'+str(cntr)+'.npy', selclientids)
        cntr += 1
    ...
    return global_acc, global_model1, global_model2, global_model3, global_model4, global_model5, global_model


# ZallocG = createzones(9, 3)
for i in range(40):
    ZallocG = get_random_assignment(40, 5)
    # global_model = custom_model.to(cuda3)
    # global_model.load_state_dict(torch.load(model_path, map_location='cpu'))
    global_acc, global_model1, global_model2, global_model3, global_model4, global_model5, global_model = execution(i, ZallocG, global_acc1, global_model, global_model1, global_model2, global_model3, global_model4, global_model5)
